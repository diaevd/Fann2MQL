...
FANN2MQL_API int __stdcall f2M_create_standard_array(int num_layers, unsigned int *layers)
{
	/* to many networks allocated */
	if (_ann >= ANNMAX) return (-1);

	/* not accepting bogus arguments */
	if (num_layers < 2) return (-1);
	...
	_fanns[_ann] = fann_create_standard_array(num_layers, layers);
	if (_fanns[_ann] == NULL) 
	{
		/* fann_create_standard returned an error */
		_ann--;
...
FANN2MQL_API int __stdcall f2M_get_total_neurons(int ann)
{
	int neurons;
	...
	neurons = (int)fann_get_total_neurons(_fanns[ann]);

	return neurons;
}

FANN2MQL_API int __stdcall f2M_get_num_layers(int ann)
{
	int layers;
	...
	layers = (int)fann_get_num_layers(_fanns[ann]);
